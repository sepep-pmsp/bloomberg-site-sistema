...
export default function TabelaFrota({ data, stats, variant }) {
    ...
    const sortedData = React.useMemo(() => {
        ...
        return [...data].sort((a, b) => {
            if ((a[sortConfig.key] || 0) < (b[sortConfig.key] || 0)) return sortConfig.direction === 'asc' ? -1 : 1;
            if ((a[sortConfig.key] || 0) > (b[sortConfig.key] || 0)) return sortConfig.direction === 'asc' ? 1 : -1;
            return 0;
        });
    }, [data, sortConfig]);
    ...
    const fmtNumero = (value) => (value || 0).toLocaleString('pt-BR', {
        minimumFractionDigits: 3,
        maximumFractionDigits: 3,
    });

    const fmtKm = (value) => (value || 0).toLocaleString('pt-BR', {
        ...
        <div>
                        ...
                        <table className="w-full text-sm">
                                    ...
                                    {!selectedBus && (
                                        <>
                                            <th className="px-4 py-4 border-l-3 border-[var(--green-800)] cursor-pointer hover:bg-gray-50 transition-colors !font-normal" onClick={() => handleSort('co2')}>
                                                <div className="flex items-center justify-center gap-1">CO₂ {renderSortIcon('co2')}</div>
                                            </th>
                            ...
                            <tbody className="bg-white">
                                {currentData.length > 0 ? currentData.map((item, idx) => {
                                    const isRedEmissions = !isMinus && ((item.co2 > stats.co2) || (item.mp > stats.mp) || (item.nox > stats.nox));
                                    ...
                                    const badgeClass = clsx(
                                        'px-4 py-1.5 rounded-full text-xs font-bold text-white text-center inline-block min-w-[90px] min-h-[28px] border-2',
                                        isMinus ? 'bg-[#B9B9B9] border-[#B9B9B9]' : 'bg-[#8A1B1B] border-[#8A1B1B]'
                                    );
                                    ...
                                    return (
                                        <tr key={`${item.id_onibus}-${idx}`} className={rowClass} onClick={() => setSelectedBus(item)}>
                                            ...
                                            <td className="px-6 py-4" style={{ fontFamily: 'var(--font-heading-secondary)' }}>
                                                <span className={lineBadgeClass}>
                                                    {item.linha ? `Linha ${item.linha}` : `Ônibus ${item.id_onibus}`}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 text-gray-700" style={{ fontFamily: 'var(--font-heading-secondary)' }}>{fmtKm(item.km_rodados)}</td>
                                            ...
                                            {!selectedBus && (
                                                <>
                                                    <td className={`${pollutionCellClass} border-l-3 border-[var(--green-800)]`} style={{ fontFamily: 'var(--font-heading-secondary)' }}>{fmtNumero(item.co2)}</td>
                                                    <td className={pollutionCellClass} style={{ fontFamily: 'var(--font-heading-secondary)' }}>{fmtNumero(item.mp)}</td>
                                                    <td className={`${pollutionCellClass} border-r-3 border-[var(--green-800)]`} style={{ fontFamily: 'var(--font-heading-secondary)' }}>{fmtNumero(item.nox)}</td>
            ...
            {totalPages > 1 && (
                <div className="mt-8 flex items-center justify-center gap-2">
                    <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-3 py-2 rounded bg-[#0A290F] text-white disabled:opacity-40">
                    ...
                    {getPageNumbers().map((page, idx) => (
                        <button
                            ...
                            className={clsx(
                                ...
                                currentPage === page ? 'bg-[#CEFA05] text-[#0A290F]' : 'bg-white text-[#0A290F] border border-gray-200',
                    ...
                    <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="px-3 py-2 rounded bg-[#0A290F] text-white disabled:opacity-40">
