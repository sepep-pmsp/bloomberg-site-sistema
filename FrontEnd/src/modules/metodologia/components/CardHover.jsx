...
export default function CardHover({ items }) {
    ...
    return (
        <div className="relative w-full flex items-center justify-center overflow-hidden h-[62%]">
            <div className="flex flex-wrap justify-center gap-10 z-10 h-full relative top-40">
                {cardItens.map(item => (
                    <div key={item.id} className="group relative w-[300px] flex items-start cursor-pointer justify-center overflow-hidden">
                        ...
                        <div className="absolute top-48 left-8 md:h-1 md:group-hover:h-170 w-60 bg-[var(--green-600)] rounded-2xl h-auto px-6 py-5 text-[var(--color-primary-light)] translate-y-[-80px] transition-all duration-500 ease-out shadow-2xl flex flex-col justify-end overflow-y-auto group-hover:shadow-2xl">
            ...
            <div className="absolute top-1/4 w-full -translate-y-1/2 z-0 pointer-events-none">
                <div className="absolute top-1/2 -left-4 w-full -translate-y-1/2 z-0 pointer-events-none -scale-x-100">
