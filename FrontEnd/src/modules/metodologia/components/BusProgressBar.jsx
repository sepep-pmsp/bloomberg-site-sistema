...
export default function BusProgressBar({ current, max, intermediate }) {
  const currentValue = current?.value || 0;
  const maxValue = max?.value || 0; // 1 para não dividir por zero
  const intermediateValue = intermediate?.value || 0;

  const currentLabel = current?.label || "veículos já substituídos!";
  const maxLabel = max?.label || "veículos eletrificados<br/>até 2028";
  const intermediateLabel = intermediate?.label || "veículos eletrificados<br/>em 2025";
    ...
    <div className="w-full relative flex flex-col justify-start items-center flex-nowrap mt-24 gap-15 py-8">
      <div className="relative w-full">
        ...
        <div className='flex justify-between w-full items-center'>
            <div style={{ left: `${(intermediateValue / maxValue) * 100}%`, transform: 'translateX(10%)'}}>
                <div className="bg-[#1D5D42] text-white py-4 rounded-b-2xl shadow-lg text-center w-55 relative">
                    <h2 className="!text-lg">
                        {intermediateValue}
                        <span className="!text-lg font-heading-primary !font-normal ml-2" dangerouslySetInnerHTML={{ __html: intermediateLabel }}/>
                    </h2>
                </div>
            </div>
            <div style={{ left: `${(intermediateValue / maxValue) * 100}%`, transform: 'translateX(-10%)'}}>
                <div className="bg-[#1D5D42] text-white py-4 rounded-b-2xl shadow-lg text-center w-55 relative">
                    <h2 className="!text-lg">
                        {maxValue}
                        <span className="!text-lg font-heading-primary !font-normal ml-2" dangerouslySetInnerHTML={{ __html: maxLabel }}/>
                    </h2>
                </div>
            </div>
        </div>
      ...
      <div className="flex flex-col items-center justify-center relative">
        <div className="absolute -top-37 w-full max-w-3xl h-[320px] pointer-events-none flex justify-center z-20 max-md:-top-32">
