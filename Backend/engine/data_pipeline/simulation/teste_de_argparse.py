...
def simMonteCarlo(df_consume_tabele, Y, days, N=2000):
    ...
    emissions_pool_co2 = df_filtered['emissao_co2'].to_numpy()
    emissions_pool_nox = df_filtered['emissao_nox'].to_numpy()
    emissions_pool_mp = df_filtered['emissao_mp'].to_numpy()
...
def main():
    ...
    try:
        ...
        df = df[df['distancia_percorrida'] >= 15 ]  # Filtrando para considerar apenas os ônibus que percorreram mais de 15 km, ajuste conforme a necessidade do seu cenário
        # Filtrando registros com distância percorrida maior que zero
        df = remove_outliers_mad(df, 'emissao_co2')
        df = remove_outliers_mad(df, 'emissao_nox')
        df = remove_outliers_mad(df, 'emissao_mp')
